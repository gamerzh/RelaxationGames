...
public class LevelUpAnim extends AnimationSprite {
	...
	public LevelUpAnim(IBitmapRes bitmapRes, int pX, int pY, int width,
			int height) {
		...
		if (isPlay) {
			animRect.set(
					...
					(int) ((EngineOptions.getScreenHeight() + (-2 * pathy + ANIM_HEIGHT
							* animScale)
							* EngineOptions.getScreenScaleY()) / 2));
			Bitmap bitmap = bitmapRes.getBitmap(FishGameRes.KEY_GAME_LEVEL_UP);
	...
	private static final int STEP_ANIM_FOUR = 3;
	...
	protected void onUpdateSelf(float secondsElapsed) {
		if (isPlay) {
			...
			case STEP_ANIM_THREE:
				if (animRect.left < 30 && animRect.top < 50) {
					anim_step = STEP_ANIM_FOUR;
				} else {
					pathx += SPEED;
					pathy += SPEED * EngineOptions.getRealScreenHeight()
	...
	public void playLevelUp(int goldNum) {
		setVisible(true);
		isPlay = true;
		this.godNum = goldNum;
	}
